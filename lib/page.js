...
function usernameFromUrl(value) {
  try {
    ...
    const rawUsername = parts.at(-1) || url.hostname.replace(/^www\./, "");
...
export function renderMemePage(
  ...
  const ownerUsername = usernameFromUrl(meme.ownerUrl);
  const safeSuggestionFormUrl = safeHttpUrl(suggestionFormUrl);
  const suggestionButton = (placementClass) => safeSuggestionFormUrl
    ? `<a class="button button-accent ${placementClass}" href="${escapeHtml(safeSuggestionFormUrl)}" target="_blank" rel="noopener noreferrer">
         Запропонувати мем
       </a>`
    : "";
  const suggestionSection = safeSuggestionFormUrl
    ? `<nav class="actions page-end-actions mobile-suggestion-actions" aria-label="Запропонувати свій мем">
         ${suggestionButton("mobile-suggestion-button")}
       </nav>`
    : "";
  ...
  return documentShell({
    ...
    body: `<main class="shell">
      ...
      <div class="meme-meta">
        <a class="button button-secondary author-button" href="${escapeHtml(meme.ownerUrl)}" target="_blank" rel="noopener noreferrer">
          Запропоновано: ${escapeHtml(ownerUsername)}
        </a>
        <a class="button button-secondary meme-source-link" href="${escapeHtml(meme.driveUrl)}" target="_blank" rel="noopener noreferrer">
          Завантажити
        </a>
        ${suggestionButton("desktop-suggestion-button")}
      </div>
      ...
      <section class="soundtrack" aria-labelledby="soundtrack-title">
        <div class="soundtrack-heading">
          ...
          <h1 id="soundtrack-title">Відчуй емоцію з піснею</h1>
        </div>
        ${player}
      </section>
