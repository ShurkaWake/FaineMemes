...
export function rowsToMemes(rows) {
  ...
  if (idColumn === -1 || memeColumn === -1 || musicColumn === -1) {
    throw new Error(
      'У таблиці мають бути колонки "id", "link to meme" та "youtube music link".',
    );
  }
  ...
  for (const [rowIndex, row] of rows.slice(1).entries()) {
    ...
    seenIds.add(id);
    memes.push({ id, driveUrl, musicUrl });
  }
